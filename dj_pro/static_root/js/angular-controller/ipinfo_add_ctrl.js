...
function($scope, Place, Ip_Info, Project_Info, Login_Mode, Database_Info, User, ipCookie, $http, $window) {
    ...
    $scope.clone = {};
    $scope.clone.machine_name = '';
    $scope.clone.Eth0Mac = '';
    $scope.clone.Eth1Mac = '';
    $scope.clone_data = function(){
	console.log($scope.clone);
	ipdata = $scope.IpData;
	project = projects;
	secret = secrets;
	data_info = data_infos;
	ipdata.machine_name = $scope.clone.machine_name;
	for(i in $scope.ip_infos){
	    console.log($scope.ip_infos[i].outerip +'=='+ $scope.clone.outerip);
	    if($scope.ip_infos[i].outerip == $scope.clone.outerip){
		$scope.alert_msg = '外网IP地址重复';
		return;
	    };
	};
	ipdata.outerip = $scope.clone.outerip;
	ipdata.innerip = $scope.clone.innerip;
	ipdata.Eth0Mac = $scope.clone.Eth0Mac;
	ipdata.Eth1Mac = $scope.clone.Eth1Mac;
	$http.post('/ipinfo/ip_info_api/', ipdata)
	.success(function(result){
	    clone_id = result.id;
	    for(i in project){
	        project[i].ip_info = clone_id;
	        delete project[i].id;
		console.log(project[i]);
	        $http.post('/ipinfo/project_info_api/', project[i])
	        .success(function(result){
		    console.log(result);
	        }).error(function(err){
        	    console.log('error');
	    ...
	    for(i in secret){
	        secret[i].ip_info = clone_id;
	        delete secret[i].id;
		console.log(secret[i]);
	        $http.post('/ipinfo/login_mode_api/', secret[i])
	        .success(function(result){
		    console.log(result);
	        }).error(function(err){
        	    console.log('error');
	    ...
	    for(i in data_info){
	        data_info[i].ip_info = clone_id;
	        delete data_info[i].id;
		console.log(data_info[i]);
	        $http.post('/ipinfo/database_info_api/', data_info[i])
	        .success(function(result){
		    console.log(result);
	        }).error(function(err){
        	    console.log('error');
